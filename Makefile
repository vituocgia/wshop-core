...
build_sandbox:
	...
	sandbox/manage.py wshop_import_catalogue sandbox/fixtures/*.csv
	sandbox/manage.py wshop_import_catalogue_images sandbox/fixtures/images.tar.gz
	sandbox/manage.py wshop_populate_countries --initial-only
...
sandbox_image:
	docker build -t wshop-core-sandbox:latest .
...
coverage: venv
	$(PYTEST) --cov=wshop --cov-report=term-missing
...
messages:
	...
	cd src/wshop; django-admin.py makemessages -a

compiledmessages:
	# Compile the gettext files
	cd src/wshop; django-admin.py compilemessages
...
todo:
	...
	# Wshop dropping support for a Django version)
	-grep -rnH TODO *.txt
	-grep -rnH TODO src/wshop/apps/
	-grep -rnH "django.VERSION" src/wshop/apps
