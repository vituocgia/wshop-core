=======================
Wshop 0.6 release notes
=======================

:release: 2014-01-08

It took a while but it's finally here: welcome to Wshop 0.6!

These release notes cover the `new features`_ as well as `backwards incompatible changes`_ 
that you'll want to be aware of when upgrading from Wshop 0.5 or
...
When upgrading your Wshop site, make sure you study both the `backwards
...
.. _`new features`: `What's new in Wshop 0.6?`_
...
The biggest change in Wshop 0.6 is the reworking of `pricing and availability`_, which 
...
partner wrappers will be removed for Wshop 0.7.

Wshop 0.6 also introduces better support for marketplace-like functionality
...
`Wshop now supports Django 1.5`_ and its custom user model.  This has been only
tested in the context of starting a new Wshop project with a custom model.
Switching from a separate "profile" model to the new system is not recommended
at this point.

Wshop also supports Django 1.6 although this is considered experimental at this
stage.  It's possible there are still some incompatibilities that haven't been
teased out just yet.

Other notable new features include:

* A feature-rich `demo site`_ that illustrates how Wshop can be customised.  It
  uses several of Wshop's many extensions such as wshop-paypal_,
  wshop-core-datacash_ and wshop-stores_.  It is intended as a
  reference site for Wshop.
...
Also, to help justify Tangent's sponsorship of Wshop, 
a simple `tracking mechanism`_ has been introduced to keep track of which sites
use Wshop.

.. _`Wshop now supports Django 1.5`: `django_support`_
...
.. _`tracking mechanism`: `Tracking Wshop sites`_

What's new in Wshop 0.6?
========================
...
These changes allow Wshop to dynamically determine prices, partner and availability 
...
* Wshop's core :class:`~wshop.core.application.Application` class now supports
...
* The dashboard's menus are now built dynamically. If the current user does
  not have access to some views in :ref:`WSHOP_DASHBOARD_NAVIGATION`, they will
  be omitted in the menu returned by
  :meth:`wshop.apps.dashboard.nav.create_menu`.
...
* The relation :attr:`wshop.apps.partner.abstract_models.AbstractPartner.users` was not
  used by core Wshop prior 0.6. It is now used to model access for the
  permission-based dashboard.
...
concrete versions.  This brings them inline with other Wshop apps and allows
...
The :class:`~wshop.apps.checkout.views.PaymentDetailsView` checkout view has
been restructured for flexibility.  There is a new
:meth:`~wshop.apps.checkout.views.PaymentDetailsView.build_submission` method
...
Wshop now ships with a demo site along side the sandbox site.  While the sandbox
is a minimal Django project that uses Wshop with all its defaults, the demo site
is a more realistic example of an Wshop project.  It has a custom skin and makes
many alterations to the default Wshop behaviour.  

It's features include:

* A range of different product types: books, downloads, clothing
* PayPal Express integration using wshop-paypal_
* Datacash integration using wshop-core-datacash_
...
is available at http://demo.wshopcommerce.com.
...
Wshop now supports Django 1.5 and, experimentally, 1.6.

Specifically, Wshop supports `custom user models`_, the headline new feature in Django
1.5.  These can be used standalone or with a one-to-one profile model: Wshop's
...
instance, Wshop's default auth backend requires the user model to have an email
and password field.  New Wshop projects are encouraged to use the provided
...
* The postcode field is now validated in the model's 
  :meth:`~wshop.apps.address.abstract_models.AbstractAddress.clean` method to
  ensure it is valid for the selected country.

* The phone number field now uses a specialist 
  :class:`~wshop.models.fields.PhoneNumberField` field class
...
:class:`~wshop.apps.payment.abstract_models.AbstractBankcard`.

An instance of this model is returned by the :attr:`~wshop.apps.payment.forms.BankcardForm.bankcard` property.
...
To this end, the core :class:`~wshop.apps.offer.models.Range` model has been
...
Wshop's search app has been reviewed and simplified.  The main view class 
(now :class:`~wshop.apps.search.views.FacetedSearchView`)
has been reworked to provide better support for faceting, which can be easily
specified using the :ref:`wshop_search_facets` setting.

The ``SuggestionsView`` has been removed as it wasn't being used.  A later
version of Wshop will include a replacement.
...
The core :class:`~wshop.apps.order.processing.EventHandler` class has been
extended.
...
See the :class:`~wshop.apps.order.processing.EventHandler` for the available
methods.

Tracking Wshop sites
~~~~~~~~~~~~~~~~~~~~

Wshop's dashboard now serves a single pixel image from one of Tangent's
servers.  This is included to gather information on which sites use Wshop,
which is an important metric for Tangent, who sponsor Wshop's development.

It can easily be disabled by setting ``WSHOP_TRACKING=False``.  If you do opt
out, please email the mailing list with any production Wshop sites you are
running.  This will help to ensure investment in Wshop's future.
...
* ``WSHOP_ALLOW_ANON_REVIEWS`` has been a documented setting since Wshop 0.4.
...
* Wshop uses a cookie to display recently displayed products. This cookie never
  ...
  ``WSHOP_RECENTLY_VIEWED_COOKIE_LIFETIME`` and
  ``WSHOP_RECENTLY_VIEWED_COOKIE_NAME``.

Backwards incompatible changes in 0.6
=====================================

There were quite a few backwards incompatible changes in Wshop 0.6.  There
shouldn't be quite as many in future Wshop releases as we approach 1.0.
...
* The ``submit`` method in
  :class:`~wshop.apps.checkout.views.PaymentDetailsView` has a new signature.
...
* The ``handle_payment`` method in
  :class:`~wshop.apps.checkout.views.PaymentDetailsView` now accepts a
  :class:`~wshop.core.prices.Price` instance instead of a Decimal for the order
  total.

* The ``handle_order_placement`` method in
  :class:`~wshop.apps.checkout.mixins.OrderPlacementMixin`
...
* The ``create_shipping_address`` method in
  :class:`~wshop.apps.checkout.mixins.OrderPlacementMixin` has changed
...
* The ``get_shipping_method`` (from
  :class:`~wshop.apps.checkout.session.CheckoutSessionMixin`) no longer
  defaults to returning free shipping if no shipping method can be looked up.

* The :class:`~wshop.apps.checkout.calculators.OrderTotalCalculator` now
  returns a :class:`~wshop.core.prices.Price` instance from a new ``calculate``
...
:class:`~wshop.apps.shipping.repository.Repository` class
...
.. warning::
    ...
    :class:`~wshop.apps.shipping.methods.FixedPrice`.  If you are using methods
    that work this way, ensure you instantiate them at runtime.
    
    Shipping methods will be reworked for Wshop 0.7 to avoid these issues.
...
The :func:`~wshop.templatetags.basket_tags.basket_form` templatetag has been
...
:class:`wshop.apps.catalogue.abstract_models.AbstractProductImage` as they
...
The :class:`wshop.apps.customer.forms.EmailAuthenticationForm` form now needs to
...
:class:`~wshop.apps.offer.models.Range` model was changed to use ``through``
relationship:
...
:class:`~wshop.apps.payment.abstract_models.AbstractSource` model is now a property, not a method.
...
:class:`wshop.apps.catalogue.reviews.forms.ProductReviewForm`.
...
:class:`~wshop.apps.partner.abstract_models.AbstractPartner` model were not
used in Wshop and have been removed to avoid confusion with the newly
...
There are rather a lot of new migrations in Wshop 0.6.  They are all detailed
below.  
...
* Offer app:
    ...
    - ``0022``: Add a ``slug`` field to the :class:`~wshop.apps.offer.models.Range` model.
    - ``0023``: A data migration to populate the new range slug field. 
    - ``0024``: Add a ``is_public`` field to the :class:`~wshop.apps.offer.models.Range` model.
    - ``0025``: Add a ``description`` field to the :class:`~wshop.apps.offer.models.Range` model.
...
:class:`~wshop.apps.catalogue.abstract_models.AbstractProduct`  class have been
deprecated following the change to allow multiple stockrecords per product. 

* The :attr:`~wshop.apps.catalogue.abstract_models.AbstractProduct.has_stockrecord` property
  no longer makes sense when there can be more than one stockrecord.  It is
  replaced by
  :attr:`~wshop.apps.catalogue.abstract_models.AbstractProduct.has_stockrecords`

* The :attr:`~wshop.apps.catalogue.abstract_models.AbstractProduct.stockrecord` property is
...
* The :attr:`~wshop.apps.catalogue.abstract_models.AbstractProduct.is_available_to_buy` property
  is deprecated.  The functionality is now part of availability policies.

* The :meth:`~wshop.apps.catalogue.abstract_models.AbstractProduct.is_purchase_permitted` method
  is deprecated.  The functionality is now part of availability policies.
...
:class:`~wshop.apps.checkout.utils.CheckoutSessionData` is now deprecated in
...
:class:`~wshop.apps.checkout.mixins.OrderPlacementMixin` are deprecated as the
flow of placing an order has been changed.

* :meth:`~wshop.apps.checkout.mixins.OrderPlacementMixin.create_shipping_address_from_form_fields`
* :meth:`~wshop.apps.checkout.mixins.OrderPlacementMixin.create_shipping_address_from_user_address`
* :meth:`~wshop.apps.checkout.mixins.OrderPlacementMixin.create_user_address`

Bankcard model
~~~~~~~~~~~~~~

The :attr:`~wshop.apps.payment.abstract_models.AbstractBankcard.card_number`
is deprecated in favour of using 
:attr:`~wshop.apps.payment.abstract_models.AbstractBankcard.number`.

"Partner wrappers"
~~~~~~~~~~~~~~~~~~

Before Wshop 0.6, availability and pricing logic was encapsulated in "partner
...
``WSHOP_PARTNER_WRAPPERS`` setting.  The stockrecord model had several
...
The following properties and methods from :class:`~wshop.apps.partner.abstract_models.StockRecord` 
are deprecated and will be removed for Wshop 0.7.  These are all convenience
properties and methods that delegate to the appropriate partner wrapper.

* :attr:`AbstractStockRecord.is_available_to_buy <wshop.apps.partner.abstract_models.AbstractStockRecord.is_available_to_buy>` 
* :meth:`AbstractStockRecord.is_purchase_permitted <wshop.apps.partner.abstract_models.AbstractStockRecord.is_purchase_permitted>` 
* :attr:`AbstractStockRecord.availability_code <wshop.apps.partner.abstract_models.AbstractStockRecord.availability_code>` 
* :attr:`AbstractStockRecord.availability <wshop.apps.partner.abstract_models.AbstractStockRecord.availability>` 
* :attr:`AbstractStockRecord.max_purchase_quantity <wshop.apps.partner.abstract_models.AbstractStockRecord.max_purchase_quantity>` 
* :attr:`AbstractStockRecord.dispatch_date <wshop.apps.partner.abstract_models.AbstractStockRecord.dispatch_date>` 
* :attr:`AbstractStockRecord.lead_time <wshop.apps.partner.abstract_models.AbstractStockRecord.lead_time>` 
* :attr:`AbstractStockRecord.price_incl_tax <wshop.apps.partner.abstract_models.AbstractStockRecord.price_incl_tax>` 
* :attr:`AbstractStockRecord.price_tax <wshop.apps.partner.abstract_models.AbstractStockRecord.price_tax>` 
...
The `Wshop test support library`_ has been ported back into Wshop core.  This
...
analogous functionality from ``wshop/test/``.

.. _`Wshop test support library`: https://github.com/vituocgia/wshop-core-testsupport
