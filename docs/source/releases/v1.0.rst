=======================
Wshop 1.0 release notes
=======================

:release: 2014-11-07

Welcome to Wshop 1.0!  It's been 7 months and some 800 commits since 0.7 and
...
Also, `as you might have seen`_, the repositories for Wshop and most of its
extensions have moved to a new `Github organisation`_. This marks a change for
Wshop from being a Tangent-sponsored project to a more community-driven one,
...
that Wshop can continue to grow and prosper.  To mark this departure, this
release has been renamed from 0.8 (under which we released three beta versions)
to 1.0.

.. _`as you might have seen`: https://groups.google.com/forum/#!searchin/wshop-core/organisation/wshop-core/6H7ByzRAkRY/6055EDottBAJ
.. _`Github organisation`: https://github.com/vituocgia/wshop-core
...
* Wshop's views are now dynamically imported. This means that they can be
  overridden like most other classes in Wshop; overriding the related
  Application instance is not necessary any more which simplifies the process of
  replacing or customising a view.

* A new management command, ``wshop_fork_app``, has been introduced to make it
  easy to fork an Wshop app in order to override one of its classes.
...
Wshop 1.0 comes with support for Django 1.7 out of the box. The app refactor
and the new migration framework are both great improvements to Django. Wshop
...
upgrading Wshop both for users staying on Django 1.6 and for users upgrading to
...
the next release of Wshop. Django 1.7 has notable improvements, so with that
...
``WSHOP_DASHBOARD_NAVIGATION`` setting:
...
To help developers building sites for the US, a new example Wshop site has been
included in the repo. This customises core Wshop to treat all prices as
...
If Wshop is running with a Solr-powered search backend, the category browsing
...
the tight coupling between Haystack and Wshop. It therefore paves the way for
...
issue.  Any existing Wshop sites with custom shipping methods will need to
...
* The ``WSHOP_CURRENCY_LOCALE`` setting has been removed. The locale is now
...
* The tracking pixel now also reports the Wshop version in use. This was
  forgotten when adding tracking of the Python and Django version in 0.7.
  Total information collected now is the versions of Django, Python and Wshop.

* The tracking pixel is now served by a server run by the new Wshop
  organisation, rather than by Tangent.

* The ``WSHOP_SLUG_FUNCTION`` now accepts both string notation and a callable.
...
* An ``WSHOP_DELETE_IMAGE_FILES`` setting has been introduced which makes deleting
...
* Wshop now ships with a ``wshop_populate_countries`` management command to
...
* Wshop has a new dependency, django-tables2_. It's a handy library that helps
...
In Wshop 1.0, shipping methods are stateless services that have a method
:func:`~wshop.apps.shipping.methods.Base.calculate` that takes a basket and
...
* The :class:`~wshop.apps.order.utils.OrderCreator` class now requires the
  ``shipping_charge`` to be passed to ``place_order``.

* The signature of the :class:`~wshop.apps.checkout.calculators.OrderTotalCalculator` 
  class has changed to accept ``shipping_charge`` rather than a
  ``shipping_method`` instance.

* The signature of the
  :func:`~wshop.apps.checkout.session.CheckoutSessionMixin.get_order_totals` 
...
of providing new shipping methods. The best practice for Wshop 1.0 is to
...
capitalisation. Because of this, Wshop now disregards capitalisation when
...
.. warning::
   ...
   that Wshop considers identical. Please use the new
   ``wshop_find_duplicate_emails`` management command to check your database
...
* You should be aware that Django 1.7 now enforces uniqueness of app labels.
  Wshop dashboard apps now ship with app configs that set their app label
  to ``{oldname}_dashboard``.

* If you have forked any Wshop apps, you must add app configs to them, and
  have them inherit from the Wshop one. See the appropriate section in
...
The basket line model got a reference to the stockrecord in Wshop 0.6. The
...
database queries. Wshop 1.0 now also enforces the stockrecord by making it
...
* The ``reload_page_response`` method of 
  :class:`~wshop.apps.dashboard.orders.views.OrderDetailView`
...
- The constructor of the :class:`~wshop.apps.basket.forms.AddToBasketForm` has
...
.. _c74f57bf: https://github.com/vituocgia/wshop-core/commit/c74f57bf434661877f4d2d2259e7e7eb18b34951#diff-d200ac8746274e0307f512af886e1f3eR148
.. _8ba283e8: https://github.com/vituocgia/wshop-core/commit/8ba283e8c4239e4eff95da5e8097a17ecfadf5f5

Misc
~~~~

* The ``wshop_calculate_scores`` command has been `rewritten`_ to use the ORM
...
* Wshop has child products to model tightly coupled products, and
  ...
  We fear it adds confusion, and in the spirit of keeping Wshop core lean,
...
* ``wshop.core.loading.get_model`` now raises a ``LookupError`` instead of an
...
* Support for the ``WSHOP_OFFER_BLACKLIST_PRODUCT`` setting has been removed.
...
* The :class:`~wshop.apps.order.utils.OrderCreator` class no longer defaults to
  free shipping: a shipping method and charge have to be explicitly passed in.

* The ``Base`` shipping method class now lives in ``wshop.apps.shipping.methods``.

* The ``find_by_code`` method of the shipping ``Repository`` class has been
  removed as it is no longer used. 

* The parameters for
  :func:`wshop.apps.shipping.repository.Repository.get_shipping_methods`
...
* ``wshop.apps.shipping.Scales`` has been renamed and moved to
  ``wshop.apps.shipping.scales.Scale``, and is now overridable.
...
* Wshop's ``models.py`` files now define ``__all__``, and it's dynamically
  ...
  star imports like ``from wshop.apps.catalogue.models import *``. You will
...
.. _rewritten: https://github.com/vituocgia/wshop-core/commit/d8b4dbfed17be90846ea4bc47b5f7b39ad944c24

Migrations
~~~~~~~~~~

* South is no longer a dependency. This means it won't get installed
  automatically when you install Wshop. If you are on Django 1.6 and want to
...
* Only South >= 1.0 is supported: South 1.0 is a backwards compatible release
  ...
  of South will look in the wrong directories and will break with this Wshop
  release.

* Rename your South ``migrations`` directories. To avoid
  clashes between Django's and South's migrations, you should rename
  all your South migrations directories (including those of forked Wshop apps)
...
* If you're upgrading to Django 1.7, you
  ...
  apps. For any forked Wshop apps, you will need to copy Wshop's initial
  migrations into your emptied ``migrations`` directory first, because Wshop's
...
.. note::
    ...
    Wshop 1.0 ships with a set of new initial migrations for Django's new
...
* ``wshop.apps.catalogue.abstract_models.AbstractProduct.has_stockrecord``
* ``wshop.apps.catalogue.abstract_models.AbstractProduct.stockrecord``
* ``wshop.apps.catalogue.abstract_models.AbstractProduct.is_available_to_buy``
* ``wshop.apps.catalogue.abstract_models.AbstractProduct.is_purchase_permitted``
* ``wshop.apps.catalogue.views.get_product_base_queryset``
* ``wshop.apps.partner.abstract_models.AbstractStockRecord.is_available_to_buy``
* ``wshop.apps.partner.abstract_models.AbstractStockRecord.is_purchase_permitted``
* ``wshop.apps.partner.abstract_models.AbstractStockRecord.availability_code``
* ``wshop.apps.partner.abstract_models.AbstractStockRecord.availability``
* ``wshop.apps.partner.abstract_models.AbstractStockRecord.max_purchase_quantity``
* ``wshop.apps.partner.abstract_models.AbstractStockRecord.dispatch_date``
* ``wshop.apps.partner.abstract_models.AbstractStockRecord.lead_time``
* ``wshop.apps.partner.abstract_models.AbstractStockRecord.price_incl_tax``
* ``wshop.apps.partner.abstract_models.AbstractStockRecord.price_tax``
* ``wshop.apps.payment.abstract_models.AbstractBankcard.card_number``

These classes have been removed:

* ``wshop.apps.partner.prices.DelegateToStockRecord``
* ``wshop.apps.partner.availability.DelegateToStockRecord``
* ``wshop.apps.payment.utils.Bankcard``
