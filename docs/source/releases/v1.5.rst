=======================
Wshop 1.5 release notes
=======================

:release: 2017-08-10

Welcome to Wshop 1.5
...
Wshop 1.5 is compatible with Django 1.8, 1.10 and 1.11 as well as Python 2.7,
...
These methods/modules have been removed:
 ...
 - ``wshop.core.logging.handlers.EnvFileHandler()``
 - The ``ellipses_page_range`` templatetag. See `django-rangepaginator`_ for an
   alternative.
 - ``wshop.test.decorators`` module.
 - ``wshop.core.utils.compose`` function.
 - ``wshop.apps.customer.auth_backends.Emailbackend``. Use
   ``wshop.apps.customer.auth_backends.EmailBackend`` instead.
 - ``wshop.apps.catalogue.search_handlers.ProductSearchHandler``. Use
 - ``wshop.apps.catalogue.search_handlers.SolrProductSearchHandler`` instead.
...
~~~~~~~~~~~~~
 ...
 - Changed product alerts to be sent using the Communication Event framework.
   The old email templates will continue to be supported until Wshop 2.0.

.. _incompatible_in_1.5:

Backwards incompatible changes in Wshop 1.5
-------------------------------------------
 ...
 - Formset classes moved to the separate modules, if you import them directly -
   please update location or use :meth:`wshop.core.loading.get_classes` for
  ...
  - ``BaseBasketLineFormSet``, ``BasketLineFormSet``, ``BaseSavedLineFormSet``,
    ...
    ``wshop.apps.basket.formsets`` module;
  - ``BaseStockRecordFormSet``, ``StockRecordFormSet``,
    ...
    ``wshop.apps.dashboard.catalogue.formsets``;
  - ``OrderedProductFormSet`` moved to
    ``wshop.apps.dashboard.promotions.formsets``; - ``LineFormset`` moved to
    ``wshop.apps.wishlists.formsets``.
 ...
 - ``WSHOP_CURRENCY_FORMAT`` setting changed to dictionary form in order to
 ...
 - ``GOOGLE_ANALYTICS_ID`` and ``USE_LESS`` settings were renamed into
   ``WSHOP_GOOGLE_ANALYTICS_ID`` and ``WSHOP_USE_LESS`` respectively in order
   to keep all Wshop settings under common namespace.
 ...
 - Offer ``Applicator`` is now loaded from the ``offer.applicator`` module,
   ...
   in the next Wshop versions.

 - ``wshop.forms.fields.ExtendedURLField`` no longer accepts a ``verify_exists``
   argument.
