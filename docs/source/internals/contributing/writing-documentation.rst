...
The docs are built by calling ``make docs`` from your Wshop directory.
...
* ``internals/`` contains everything related to Wshop itself, e.g. contributing
  guidelines or design philosophies.
* ``ref/`` is the reference documentation, esp. consisting of
* ``ref/apps/`` which should be a guide to each Wshop core app, explaining it's
...
* ``topics/`` will contain "meta" articles, explaining how things tie together
  over several apps, or how Wshop can be combined with other solutions.
...
Style guides
------------
Wshop currently does not have it's own style guide for writing documentation.
