...
scratch and have decided to use Wshop.  Let's call this shop 'frobshop'
...
Install Wshop and its dependencies
==================================

Install Wshop (which will install Django as a dependency), then create the
project:

.. code-block:: bash

    $ mkvirtualenv wshop
    $ pip install wshop-core
...
recommended to install Wshop in a virtualenv.
...
First, edit your settings file ``frobshop.frobshop.settings.py`` to import all of Wshop's default settings.

.. code-block:: django

    from wshop.defaults import *


Now modify your ``TEMPLATES`` to include the main Wshop template directory and add the extra
context processors.
...
Wshop's core apps. Also set ``SITE_ID``:
...
Note that Wshop requires ``django.contrib.flatpages`` which isn't
...
.. tip::
    ...
    Wshop's default templates use django-compressor_ and django-widget-tweaks_
...
Next, add ``wshop.apps.basket.middleware.BasketMiddleware`` and
...
.. _sandbox settings: https://github.com/vituocgia/wshop-core/blob/master/sandbox/settings.py#L102


URLs
====

Alter your ``frobshop/urls.py`` to include Wshop's URLs. You can also include
the Django admin for debugging purposes. But please note that Wshop makes no
...
Wshop uses Haystack to abstract away from different search backends.
...
Wshop includes a sample schema to get started with Solr. More information can
be found in the
...
Wshop ships with migrations. Django's migration framework will detect them
...
You should now have an empty, but running Wshop install that you can browse at
http://localhost:8000.
...
The default checkout process requires a shipping address with a country.  Wshop
...
package. Wshop ships with a management command to parse that data:

.. code-block:: bash

    $ pip install pycountry
    [...]
    $ python manage.py wshop_populate_countries
...
Every Wshop deployment needs at least one
:class:`product class <wshop.apps.catalogue.abstract_models.AbstractProductClass>`
and one
:class:`fulfillment partner <wshop.apps.partner.abstract_models.AbstractPartner>`.
These aren't created automatically as they're highly specific to the shop you
want to build.

When managing your catalogue you should always use the Wshop dashboard, which
...
The order management in Wshop relies on the order pipeline that
defines all the statuses an order can have and the possible transitions
for any given status. Statuses in Wshop are not just used for an order
...
changing it is fairly straightforward in Wshop. The pipeline is defined in
your ``settings.py`` file using the ``WSHOP_ORDER_STATUS_PIPELINE`` setting.
You also need to specify the initial status for an order and a line item in
``WSHOP_INITIAL_ORDER_STATUS`` and ``WSHOP_INITIAL_LINE_STATUS``
respectively.

To give you an idea of what an order pipeline might look like take a look
at the Wshop sandbox settings:
