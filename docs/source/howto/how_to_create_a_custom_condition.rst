...
Wshop ships with several condition models that can be used to build offers.
However, occasionally a custom condition can be useful.  Wshop lets you build a
...
Custom condition classes must be proxy models, subclassing Wshop's main
:class:`~wshop.apps.offer.models.Condition` class.
...
To make this condition available to be used in offers, do the following::
    ...
    from wshop.apps.offer.custom import create_condition
