...
Wshop comes with a pre-configured dashboard navigation that gives you access
...
``WSHOP_DASHBOARD_NAVIGATION`` setting in you settings file.
...
``access_fn`` parameter for the current node, so that Wshop is able to
...
Wshop uses `Font Awesome`_ for its icons which makes it very simple to add
...
If no ``'access_fn'`` is specified, ``WSHOP_DASHBOARD_DEFAULT_ACCESS_FUNCTION``
is used.