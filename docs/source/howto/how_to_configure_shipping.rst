...
Wshop can handle all of these shipping scenarios.

Shipping in Wshop
~~~~~~~~~~~~~~~~~

Configuring shipping charges requires overriding Wshop's core 'shipping' app
...
:func:`~wshop.apps.shipping.repository.Repository.get_shipping_methods` method,
...
.. note::
    ...
    Wshop's checkout process includes a page for choosing your shipping method.
...
.. code-block:: python
   ...
   from wshop.apps.shipping import repository
   from . import methods
...
methods subclass
:class:`~wshop.apps.shipping.methods.Base`, which stubs this API.
...
Wshop ships with several re-usable shipping methods which can be used as-is, or
subclassed and customised:

* :class:`~wshop.apps.shipping.methods.Free` - no shipping charges

* :class:`~wshop.apps.shipping.methods.FixedPrice` - fixed-price shipping charges.
  Example usage:
...
:class:`~wshop.apps.shipping.abstract_models.AbstractWeightBased`
