...
Wshop is designed to be very flexible around payment.  It supports paying for an
...
* ``SourceType`` - This is the type of payment source used (eg PayPal, DataCash).  As part of setting up
  a new Wshop site you would create a SourceType for each of the payment
...
wshop-paypal_ extension.  Please use that as a reference.

See also the sandbox for wshop-core-datacash_ which follows a similar pattern.


.. _wshop-paypal: https://github.com/wshop-core/wshop-paypal/tree/master/sandbox
.. _wshop-core-datacash: https://github.com/vituocgia/wshop-core-datacash/tree/master/sandbox

Integration into checkout
-------------------------

By default, Wshop's checkout does not provide any payment integration as it is
...
successful, then nothing needs to be returned.  However, Wshop defines a few
common exceptions which can occur:

* ``wshop.apps.payment.exceptions.RedirectRequired``  For payment integrations
  that require redirecting the user to a 3rd-party site.  This exception class
  has a ``url`` attribute that needs to be set.

* ``wshop.apps.payment.exceptions.UnableToTakePayment`` For *anticipated* payment
  problems such as invalid bankcard number, not enough funds in account - that kind
  of thing.

* ``wshop.apps.payment.exceptions.UserCancelled`` During many payment flows,
...
* ``wshop.apps.payment.exceptions.PaymentError``  For *unanticipated* payment
...
* Create the appropriate ``wshop.apps.payment.models.Source`` instance and pass
...
For example::
    ...
    from wshop.apps.checkout import views
    from wshop.apps.payment import models


    # Subclass the core Wshop view so we can customise
