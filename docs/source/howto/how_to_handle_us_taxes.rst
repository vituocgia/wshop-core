...
address has been entered.  This scenario requires two changes from core Wshop.
...
is based in the US.  Wshop provides a :class:`~wshop.apps.partner.strategy.US`
strategy class that uses the :class:`~wshop.apps.partner.strategy.DeferredTax`
...
Second, the :class:`~wshop.apps.checkout.session.CheckoutSessionMixin`
should be overridden within your project to apply taxes
to the submission.

.. code-block:: python

    from wshop.apps.checkout import session
