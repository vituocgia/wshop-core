...
Wshop provides only a skeleton for building your order processing pipeline on
...
There are two relevant Wshop apps to order processing.
...
Wshop models order processsing through events.  There are three types to be
aware of:
...
Most Wshop shops will want to customise the ``EventHandler`` class from the
order app.  This class is intended to handle all events and perform the
appropriate actions.  The main public API is

.. autoclass:: wshop.apps.order.processing.EventHandler
...
It can be useful to use order and line statuses too.  Wshop provides some helper
methods to make this easier.

.. autoclass:: wshop.apps.order.abstract_models.AbstractOrder
   :members: pipeline, all_statuses, available_statuses, set_status
   :noindex:

.. autoclass:: wshop.apps.order.abstract_models.AbstractLine
   :members: pipeline, all_statuses, available_statuses, set_status
   :noindex:
...
the functionality described here is not in Wshop.  However, Wshop provides the
