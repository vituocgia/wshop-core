...
def metadata(request):
    ...
    """
    return {'shop_name': settings.WSHOP_SHOP_NAME,
            'shop_tagline': settings.WSHOP_SHOP_TAGLINE,
            'homepage_url': settings.WSHOP_HOMEPAGE,
            'language_neutral_url_path': strip_language_code(request),
            # Fallback to old settings name for backwards compatibility
            'use_less': (getattr(settings, 'WSHOP_USE_LESS', None) or
                         getattr(settings, 'USE_LESS', False)),
            'google_analytics_id': (getattr(settings, 'WSHOP_GOOGLE_ANALYTICS_ID', None) or
