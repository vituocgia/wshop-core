...
from wshop.core.exceptions import (
...
# from one Wshop module to another, we look into the dictionary below
# for the moved items during loading.
MOVED_ITEMS = {
    'wshop.apps.basket.forms': (
        'wshop.apps.basket.formsets', ('BaseBasketLineFormSet', 'BasketLineFormSet',
                                       'BaseSavedLineFormSet', 'SavedLineFormSet')
    ),
    'wshop.apps.dashboard.catalogue.forms': (
        'wshop.apps.dashboard.catalogue.formsets', ('BaseStockRecordFormSet',
                                                    'StockRecordFormSet',
    ...
    'wshop.apps.dashboard.promotions.forms': (
        'wshop.apps.dashboard.promotions.formsets', ('OrderedProductFormSet',)
    ),
    'wshop.apps.wishlists.forms': (
        'wshop.apps.wishlists.formsets', ('LineFormset',)
    )
}


def get_class(module_label, classname, module_prefix='wshop.apps'):
    """
...
def get_class_loader():
    return import_string(settings.WSHOP_DYNAMIC_CLASS_LOADER)


def get_classes(module_label, classnames, module_prefix='wshop.apps'):
...
def default_class_loader(module_label, classnames, module_prefix):
    ...
    Examples:
        ...
        wshop.apps.dashboard.catalogue.forms.ProductForm
        ...
        [wshop.apps.dashboard.catalogue.forms.ProductForm,
         wshop.apps.dashboard.catalogue.forms.StockRecordForm]
    ...
    # import from Wshop package (should succeed in most cases)
    # e.g. 'wshop.apps.dashboard.catalogue.forms'
    wshop_module_label = "%s.%s" % (module_prefix, module_label)
    wshop_module = _import_module(wshop_module_label, classnames)

    # returns e.g. 'wshop.apps.dashboard.catalogue',
    ...
    if installed_apps_entry.startswith('%s.' % module_prefix):
        # The entry is obviously an Wshop one, we don't import again
        local_module = None
    else:
    ...
    wshop_move_item = MOVED_ITEMS.get(wshop_module_label, None)
    if wshop_move_item:
        wshop_move_module_label = wshop_move_item[0]
        wshop_move_classnames = wshop_move_item[1]
        wshop_moved_classnames = list(set(wshop_move_classnames).intersection(classnames))
        if wshop_moved_classnames:
            warnings.warn(
                'Classes %s has recently moved to the new destination module - %s, '
                'please update your imports.' % (', '.join(wshop_moved_classnames),
                                                 wshop_move_module_label),
                DeprecationWarning, stacklevel=2)
            wshop_move_module = _import_module(wshop_move_module_label, classnames)
        else:
            wshop_move_module = None
    else:
        wshop_move_module = None

    if wshop_module is wshop_move_module is local_module is None:
    ...
    return _pluck_classes([local_module, wshop_module, wshop_move_module], classnames)
...
def get_profile_class():
    ...
    # makes sense for Wshop to continue to use it. Projects built on Django
...
def feature_hidden(feature_name):
    """
    Test if a certain Wshop feature is disabled.
    """
    return (feature_name is not None and
            feature_name in settings.WSHOP_HIDDEN_FEATURES)
...
def is_model_registered(app_label, model_name):
    ...
    register Wshop models if they aren't overridden by a forked app.
    """
    try:
