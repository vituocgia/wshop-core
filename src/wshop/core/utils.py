...
def default_slugifier(value):
    """
    Wshop's default slugifier function. Uses Django's slugify function.
    """
    return django_slugify(value, allow_unicode=settings.WSHOP_SLUG_ALLOW_UNICODE)
...
def slugify(value):
    ...
    if not settings.WSHOP_SLUG_ALLOW_UNICODE:
        value = unidecode(six.text_type(value))

    value = slugifier(six.text_type(value))
    # Remove stopwords
    for word in settings.WSHOP_SLUG_BLACKLIST:
...
def format_datetime(dt, format=None):
    ...
    if is_naive(dt):
        ...
        logging.warning(
            "wshop.core.utils.format_datetime received native datetime")
    else:
...
def get_default_currency():
    ...
    WSHOP_DEFAULT_CURRENCY as something it needs to generate a migration for.
    """
    return settings.WSHOP_DEFAULT_CURRENCY
