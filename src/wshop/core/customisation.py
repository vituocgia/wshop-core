...
def inherit_app_config(local_app_path, app_package, app_label):
    ...
    create_file(
        ...
        "from wshop.apps.{app_label} import config\n\n\n"
...
def fork_app(label, folder_path, logger=None):
    """
    Create a custom version of one of Wshop's apps
    ...
    valid_labels = [x.replace('wshop.apps.', '') for x in wshop.WSHOP_CORE_APPS
                    if x.startswith('wshop')]
    if label not in valid_labels:
        raise ValueError("There is no Wshop app that matches '%s'" % label)
    ...
    wshop_app_path = join(wshop.__path__[0], 'apps', label_folder)
    if exists(os.path.join(wshop_app_path, 'admin.py')):
        logger.info("Creating admin.py")
        create_file(join(local_app_path, 'admin.py'),
                    "from wshop.apps.%s.admin import *  # noqa\n" % label)

    logger.info("Creating app config")
    inherit_app_config(local_app_path, app_package, label)

    # Only create models.py and migrations if it exists in the Wshop app
    wshop_models_path = join(wshop_app_path, 'models.py')
    if exists(wshop_models_path):
        logger.info("Creating models.py")
        create_file(
            join(local_app_path, 'models.py'),
            "from wshop.apps.%s.models import *  # noqa isort:skip\n" % label)

        migrations_path = 'migrations'
        source = join(wshop_app_path, migrations_path)
        if exists(source):
    ...
    msg = (
        ...
        "(replacing the equivalent Wshop app). This can be "
        "achieved using Wshop's get_core_apps function - e.g.:"
    ) % app_package
    snippet = (
        "  # settings.py\n"
        "  ...\n"
