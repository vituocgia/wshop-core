...
dashboard_urlpatterns = [
    url(r'^band/create/$', BandCreateView.as_view(), name='wshop-band-create'),
    url(r'^band/$', BandListView.as_view(), name='wshop-band-list'),
    ...
    url(r'^band/(?P<pk>\w+)/update/$', BandUpdateView.as_view(), name='wshop-band-update'),
    # The RelatedFieldWidgetWrapper code does something funny with placeholder
    # urls, so it does need to match more than just a pk
    url(r'^band/(?P<pk>\w+)/delete/$', BandDeleteView.as_view(), name='wshop-band-delete'),
]
