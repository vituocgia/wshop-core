...
from wshop.test.factories.utils import tax_add, tax_subtract
...
class BillingAddressFactory(factory.DjangoModelFactory):
    country = factory.SubFactory('wshop.test.factories.CountryFactory')
...
class OrderFactory(factory.DjangoModelFactory):
    ...
    if hasattr(settings, 'WSHOP_INITIAL_ORDER_STATUS'):
        status = settings.WSHOP_INITIAL_ORDER_STATUS

    site_id = settings.SITE_ID
    number = factory.LazyAttribute(lambda o: '%d' % (100000 + o.basket.pk))
    basket = factory.SubFactory(
        'wshop.test.factories.BasketFactory')
...
class OrderLineFactory(factory.DjangoModelFactory):
    ...
    product = factory.SubFactory(
        'wshop.test.factories.ProductFactory')
