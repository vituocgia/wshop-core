...
class AccountSummaryView(generic.RedirectView):
    ...
    Wshop defaults to just redirecting to the profile summary page (and
    that redirect can be configured via WSHOP_ACCOUNT_REDIRECT_URL), but
    ...
    """
    pattern_name = settings.WSHOP_ACCOUNTS_REDIRECT_URL
    permanent = False
...
class LogoutView(generic.RedirectView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        for cookie in settings.WSHOP_COOKIES_DELETE_ON_LOGOUT:
...
class ProfileDeleteView(PageTitleMixin, generic.FormView):
    ...
    active_tab = 'profile'
    success_url = settings.WSHOP_HOMEPAGE
...
class EmailHistoryView(PageTitleMixin, generic.ListView):
    ...
    paginate_by = settings.WSHOP_EMAILS_PER_PAGE
...
class OrderHistoryView(PageTitleMixin, generic.ListView):
    ...
    paginate_by = settings.WSHOP_ORDERS_PER_PAGE
    model = Order
...
class AddressListView(PageTitleMixin, generic.ListView):
    ...
    paginate_by = settings.WSHOP_ADDRESSES_PER_PAGE
