...
def extract(request, response=None):
    ...
    """
    ids = []
    cookie_name = settings.WSHOP_RECENTLY_VIEWED_COOKIE_NAME
...
def add(ids, new_id):
    ...
    """
    max_products = settings.WSHOP_RECENTLY_VIEWED_PRODUCTS
    if new_id in ids:
        ids.remove(new_id)
    ids.append(new_id)
...
def update(product, request, response):
    ...
    response.set_cookie(
        settings.WSHOP_RECENTLY_VIEWED_COOKIE_NAME,
        json.dumps(updated_ids),
        max_age=settings.WSHOP_RECENTLY_VIEWED_COOKIE_LIFETIME,
        secure=settings.WSHOP_RECENTLY_VIEWED_COOKIE_SECURE,
        httponly=True)
