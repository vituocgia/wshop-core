...
from wshop.apps.order.signals import (
...
from wshop.models.fields import AutoSlugField

from . import exceptions


logger = logging.getLogger('wshop.order')
...
class AbstractOrder(models.Model):
    ...
    #: that one.
    pipeline = getattr(settings, 'WSHOP_ORDER_STATUS_PIPELINE', {})
    ...
    #: status
    cascade = getattr(settings, 'WSHOP_ORDER_STATUS_CASCADE', {})
    ...
    def verification_hash(self):
        signer = Signer(salt='wshop.apps.order.Order')
    ...
    def check_deprecated_verification_hash(self, hash_to_check):
        ...
        This must explicitly be enabled by setting WSHOP_DEPRECATED_ORDER_VERIFY_KEY,
        which must not be equal to SECRET_KEY - i.e., the project must
        have changed its SECRET_KEY since this change was applied.

        TODO: deprecate this method in Wshop 2.0, and remove it in Wshop 2.1.
        """
        old_verification_key = getattr(settings, 'WSHOP_DEPRECATED_ORDER_VERIFY_KEY', None)
        if old_verification_key is None:
            return False

        if old_verification_key == settings.SECRET_KEY:
            raise ImproperlyConfigured(
                'WSHOP_DEPRECATED_ORDER_VERIFY_KEY cannot be equal to SECRET_KEY')
...
class AbstractLine(models.Model):
    ...
    # Deprecated - will be removed in Wshop 2.0
    ...
    #: one.
    pipeline = getattr(settings, 'WSHOP_LINE_STATUS_PIPELINE', {})
