...
from wshop.apps.partner.exceptions import InvalidStockAdjustment
...
class AbstractPartner(models.Model):
    ...
    setting up an Wshop deployment. Many Wshop deployments will only have one
    fulfillment partner.
    """
    ...
    def primary_address(self):
        ...
        if len(addresses) == 0:  # intentionally using len() to save queries
            ...
            raise NotImplementedError(
                "Wshop's default implementation of primary_address only "
...
class AbstractStockRecord(models.Model):
    ...
    # renamed into `price` in Wshop 2.0.
