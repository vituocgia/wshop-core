...
def base_sqs():
    ...
    """
    sqs = SearchQuerySet()
    for facet in settings.WSHOP_SEARCH_FACETS['fields'].values():
...
class FacetMunger(object):
    ...
    def munge_field_facets(self, clean_data):
        for key, facet in settings.WSHOP_SEARCH_FACETS['fields'].items():
    ...
    def munge_query_facets(self, clean_data):
        for key, facet in settings.WSHOP_SEARCH_FACETS['queries'].items():
    ...
    def munge_query_facet(self, key, facet, clean_data):
        clean_data[key] = {
            'name': facet['name'],
            'results': []}
        # Loop over the queries in WSHOP_SEARCH_FACETS rather than the returned
