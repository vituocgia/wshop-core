...
then
  ...
  mysql -u root -e "DROP DATABASE IF EXISTS wshop_travis; CREATE DATABASE wshop_travis"
  mysql -u root -e "GRANT ALL PRIVILEGES ON wshop_travis.* TO 'travis'@'localhost' IDENTIFIED BY '';"

  echo "Creating Postgres database and user"
  psql -c "DROP ROLE IF EXISTS travis"
  psql -c "CREATE ROLE travis LOGIN PASSWORD ''"
  psql -c "DROP DATABASE IF EXISTS wshop_travis"
  psql -c "CREATE DATABASE wshop_travis"
fi
