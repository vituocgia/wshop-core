...
from wshop.apps.basket import forms, formsets
...
class TestBasketLineForm(TestCase):
    ...
    def test_enforces_max_line_quantity_for_new_product(self):
        invalid_qty = settings.WSHOP_MAX_BASKET_QUANTITY_THRESHOLD + 1
        form = self.build_form(quantity=invalid_qty)
        self.assertFalse(form.is_valid())

    @override_settings(WSHOP_MAX_BASKET_QUANTITY_THRESHOLD=10)
