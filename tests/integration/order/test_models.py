...
from wshop.apps.order.exceptions import (
...
from wshop.apps.order.signals import order_line_status_changed, order_status_changed
from wshop.test.basket import add_product
from wshop.test.contextmanagers import mock_signal_receiver
from wshop.test.factories import (
...
class OrderTests(TestCase):
    ...
    @mock.patch('wshop.apps.order.abstract_models.now')
    ...
    @override_settings(WSHOP_DEPRECATED_ORDER_VERIFY_KEY='deprecated_order_hash_secret')
    ...
    @override_settings(
        WSHOP_DEPRECATED_ORDER_VERIFY_KEY='deprecated_order_hash_secret',
        SECRET_KEY='deprecated_order_hash_secret')
    def test_check_deprecated_hash_verification_old_key_matches_new(self):
        order = OrderFactory(number='100001')
        # WSHOP_DEPRECATED_ORDER_VERIFY_KEY must not be equal to SECRET_KEY.
