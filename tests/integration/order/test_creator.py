...
from wshop.apps.catalogue.models import ProductClass, Product
from wshop.apps.checkout import calculators
from wshop.apps.offer.utils import Applicator
from wshop.apps.order.models import Order
from wshop.apps.order.utils import OrderCreator
from wshop.apps.shipping.methods import Free, FixedPrice
from wshop.apps.shipping.repository import Repository
from wshop.apps.voucher.models import Voucher
...
class TestSuccessfulOrderCreation(TestCase):
    ...
    def test_uses_default_order_status_from_settings(self):
        add_product(self.basket, D('12.00'))
        with override_settings(WSHOP_INITIAL_ORDER_STATUS='A'):
            place_order(self.creator, basket=self.basket, order_number='1234')
        order = Order.objects.get(number='1234')
    ...
    def test_uses_default_line_status_from_settings(self):
        add_product(self.basket, D('12.00'))
        with override_settings(WSHOP_INITIAL_LINE_STATUS='A'):
            place_order(self.creator, basket=self.basket, order_number='1234')
        order = Order.objects.get(number='1234')
