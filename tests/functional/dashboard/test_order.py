...
class TestOrderListSearch(WebTestCase):
    ...
    TEST_CASES = [
        ...
        (
            # Assumes that the test settings (WSHOP_ORDER_STATUS_PIPELINE)
...
class TestOrderDetailPage(WebTestCase):
    ...
    def test_allows_line_status_to_be_changed(self):
        ...
        self.assertEqual(line.status, settings.WSHOP_INITIAL_LINE_STATUS)
    ...
    def test_allows_order_status_to_be_changed(self):
        ...
        self.assertEqual(
            self.order.status, settings.WSHOP_INITIAL_ORDER_STATUS)
