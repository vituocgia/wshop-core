...
class AnonAddToBasketViewTests(WebTestCase):
    ...
    def test_cookie_is_created(self):
        self.assertTrue('wshop_open_basket' in self.response.test_app.cookies)

    def test_price_is_recorded(self):
        wshop_open_basket_cookie = unquote_cookie(self.response.test_app.cookies['wshop_open_basket'])
        basket_id = wshop_open_basket_cookie.split(':')[0]
...
class BasketThresholdTest(WebTestCase):
    ...
    def setUp(self):
        self._old_threshold = settings.WSHOP_MAX_BASKET_QUANTITY_THRESHOLD
        settings.WSHOP_MAX_BASKET_QUANTITY_THRESHOLD = 3

    def tearDown(self):
        settings.WSHOP_MAX_BASKET_QUANTITY_THRESHOLD = self._old_threshold
